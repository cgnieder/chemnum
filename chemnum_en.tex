...
marked with \expandablesign\ are expandable in an \cs*{edef} like context.
...
\begin{example}[side-by-side]
  \cmpd{q.two} and \cmpd{q.five} \par
  \cmpd{q.five,q.two} \par
  \cmpd{q.two,q.five} \par
  \cmpd[merge]{q.two,q.five}
\end{example}
...
\option{merge} then the problem vanishes as then only one label is printed.
