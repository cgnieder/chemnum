...
\setcnltx{
  ...
  url     = https://bitbucket.org/cgnieder/chemnum/ ,
  add-cmds = {
    chemnumshowdef,
    chemnumshowref,
    cmpd ,
...
the \bnd{chemmacros} bundle~\cite{bnd:chemmacros}.  It also loads the
\pkg{psfrag}~\cite{pkg:psfrag}.
...
is why version~0 is still available through the option \keyis{version}{0}.  If
...
\begin{commands}
  \command{cmpd}[\sarg\code{+}\oarg{options}\marg{list of \acsp{id}}]
  ...
  \expandable\command{subcmpdplain}[\marg{main \acs{id}}\marg{sub \acs{id}}]
    ...
    (\cf\ \cs*{texorpdfstring}\marg{\TeX}\marg{pdfstring}).  This command is
    described in section~\ref{sec:deta-comp-labels}.
  \expandable\command{submaincmpdplain}[\marg{main \acs{id}}\marg{sub \acs{id}}]
    ...
    (\cf\ \cs*{texorpdfstring}\marg{\TeX}\marg{pdfstring}).  This command is
    described in section~\ref{sec:deta-comp-labels}.
  \command{initcmpd}[\oarg{options}\marg{list of \acsp{id}}]
    Initiate compound labels.  This command can only be used in the preamble.
    It is desribed in section~\ref{sec:deta-comp-labels}.
  \expandable\command{cmpdproperty}[\marg{\acs{id}}\marg{property}]
    Get the associated property \meta{property} of compound
    \meta{\acs{id}}. This command is described in detail in
    section~\ref{sec:deta-comp-labels}.
  \expandable\command{subcmpdproperty}[\marg{main \acs{id}}\marg{sub
    \acs{id}}\marg{property}]
    Get the associated property \meta{property} of subcompound \meta{sub
      \acs{id}} of compound \meta{main \acs{id}}.  This command is described
  ...
  \command{cmpdshowdef}[\marg{\acs{id}}]
    Internal command used to display \meta{\acs{id}} of a newly defined compound
    label when the option \option{show-keys} is used.  The command is
    described in section~\ref{sec:debugg-inform}.
  \command{cmpdshowref}[\marg{\acs{id}}]
    Internal command used to display \meta{\acs{id}} of a referencing compound
    label when the option \option{show-keys} is used.  The command is
    described in section~\ref{sec:debugg-inform}.
  \command{subcmpdshowdef}[\marg{main \acs{id}}\marg{sub \acs{id}}]
    Internal command used to display \meta{main \acs{id}} and \meta{sub
      \acs{id}} of a newly defined subcompound label when the option
    \option{show-keys} is used.  The command is described in
    section~\ref{sec:debugg-inform}.
  \command{subcmpdshowref}[\marg{main \acs{id}}\marg{sub \acs{id}}]
    Internal command used to display \meta{main \acs{id}} and \meta{sub
      \acs{id}} of a referencing subcompound label when the option
    \option{show-keys} is used.  The command is described in
    section~\ref{sec:debugg-inform}.
\end{commands}
...
\begin{commands}
  \command{cmpd}[\Marg{\meta{main \acs{id}}.\meta{sub \acs{id}}}]
    \meta{main \acs{id}} is the main name which stays the same, \meta{sub
      \acs{id}} varies.  This syntax means that the point \code{.}
    \emph{cannot} be a part of \meta{main \acs{id}} or \meta{sub \acs{id}}.
...
the \code{aux} file.  The command \cs{refcmpd}\oarg{options}\marg{\acs{id}}
...
It's equivalent \cs{subcmpdplain}\marg{main \acs{id}}\marg{sub \acs{id}} does
the same for sublabels.  A third sibling, \cs{submaincmpdplain}\marg{main
  \acs{id}}\marg{sub \acs{id}}, writes both the main and the sublabel.
...
\begin{commands}
  \expandable\command{cmpdproperty}[\marg{\acs{id}}\marg{property}]
    Get the associated property \meta{property} of compound \meta{\acs{id}}.  This
...
\begin{commands}
  \expandable\command{subcmpdproperty}[\marg{main \acs{id}}\marg{sub
    \acs{id}}\marg{property}]
    Get the associated property \meta{property} of subcompound \meta{sub
      \acs{id}} of compound \meta{main \acs{id}}.  This command is
    expandable.
\end{commands}
...
\option{init} \chemnum\ will either issue a warning or an error (depending on
the actual setting you chose) if a label is used \emph{that hasn't been
  initiated}.
...
key/value pairs.  Options that can be set via \cs{setchemnum} are marked with
...
\begin{options}
  ...
  \keyval{list-label-sep}{code}\Module{all}\Default{,}
    The separator that is used to separate different \meta{main \acsp{id}} in
    \cs{cmpd}.
  ...
  \keyval{style}{code}\Module{replace}\Default{\cs*{sffamily}}
    ...
    the replacement.
  \keychoice{replace-pos}{\marg{\TeX\ pos}\marg{\acs{ps}
      pos}}\Module{global}\Default{bb}
    Options for \pkg{psfrag}'s \cs{psfrag}.
  \keychoice{pos}{\marg{\TeX\ pos}\marg{\acs{ps}
  ...
  \keychoice{show-keys}{\default{true},false,def,ref}\Module{global}\Default{false}
...
\begin{options}
  \keyval{format}{formatting commands}\Default{\cs*{bfseries}}
    The default format of the labels.
\end{options}
...
\begin{options}
  \keyval{list-sep-two}{code}%
    \Default{\visualizespaces{\GetTranslation{chemnum-sep-two}}}
    The output separator between labels in a list that contains of two items.
  \keyval{list-sep-more}{code}\Default{\visualizespaces{, }}
    The output separator between labels in a list that contains of more than
    two items.
  \keyval{list-sep-last-two}{code}%
    \Default{\visualizespaces{\GetTranslation{chemnum-sep-last-two}}}
    The output separator between the last two labels in a list that contains
    of more than two items.
\end{options}
...
\begin{itemize}
  ...
  \item Export the scheme as \acs{eps} or \acs{ps} figure where you make sure
    that the tags are embedded as text strings.
  \item Include the \acs{eps} with \cs*{includegraphics}.  Right before that
...
\begin{example}[compile,exe-with={--shell-escape},runs=1,float,caption={A scheme with
...
respect to the \acs{ps} box that is replaced.  This is described in
...
\begin{example}[compile,exe-with={--shell-escape},runs=1,float,caption={A scheme with
  ...
  \begin{document}
    ...
    \replacecmpd[tag=blah]{main}% replaces TMP1
    \replacecmpd[tag=blub]{main.sub}% replaces TMP2
    \includegraphics{scheme-bla.ps}
  \end{document}
\end{example}
...
with the \acs{id} \code{showkey}: \setchemnum{show-keys}\cmpd{showkey} that
