...
% in the document you're reading right now.
...
\begin{commands}
  ...
  \expandable\command{cmpdplain}[\marg{\acs{id}}]
    ...
    to parse a list.  Its sole purpose is usage in \acs{pdf} strings
    (\cf\ \cs*{texorpdfstring}\marg{\TeX}\marg{\acs{pdf} string}).  This
  ...
  \expandable\command{submaincmpdplain}[\marg{main \acs{id}}\marg{sub \acs{id}}]
    ...
    \acs{pdf} strings (\cf\ \cs*{texorpdfstring}\marg{\TeX}\marg{\acs{pdf}
  ...
  \expandable\command{cmpdproperty}[\marg{\acs{id}}\marg{property}]
    ...
    \meta{\acs{id}}. This command is described in
    section~\ref{sec:deta-comp-labels}.
...
\begin{commands}
  \command{cmpd}[\marg{\acs{id}}]
    When \meta{\acs{id}} is used the first time, the label is created, saved
...
\begin{commands}
  \command{cmpd}[\sarg\marg{\acs{id}}]
    Declare the label \meta{\acs{id}} but don't print anything.
\end{commands}
...
\cs{labelcmpd}\oarg{options}\marg{\acs{id}}.  Both commands have shortcut
...
unordered lists of such labels\footnote{This is something to be fixed
  (hopefully) in some future version of \chemnum.}.
...
\begin{options}
  ...
  \keybool{compress}\Module{both (l)}\Default{false}
  ...
  \keyval{main-sub-sep}{code}\Module{general}\Default{.}
  ...
  \keyval{list-label-sep}{code}\Module{general}\Default{,}
    The separator that is used to separate different \meta{main \acsp{id}} in
    \cs{cmpd}.
  \keyval{sub-list-label-sep}{code}\Module{general}\Default{,}
